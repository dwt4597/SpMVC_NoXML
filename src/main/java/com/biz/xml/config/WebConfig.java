...
public class WebConfig implements WebMvcConfigurer {
	 ...
	 * @Controller @Service는 사용자(개발자)가 호출하여 사용할 클래스들에
	 * 		지정하는 방식이고
	 * @Bean은 spring(Dispatcher)가 사용할 클래스들에 부여하는
	 * 	Annotation
	 * 
	...
	public MultipartResolver multipartResolver() {
		...
		// mr 객체를 선언하고
		// multi... 인터페이스를 implement 한 Commons... 클래스의
		// 생성자로 초기화를 한 후, 
		MultipartResolver mr
			= new CommonsMultipartResolver();
		//다시 mr 객체를 Commons.. 클래스로 1차 형변환을 한 후
		// setMaxUploadSize와 같은 메서드를 호출해서
		// 기능을 사용해야 한다.
	...
	private void test() {
		...
		//권장하는 방식
		//List 인터페이스를 사용해서 선언하고
		//ArrayList를 사용해서 초기화를 하는 방식
		List<String> lList = new ArrayList<String>();
		// 1. 혹시 코드를 작성하는 중에
		// ArrayList를 LinkedList로 변경하고 싶다 할 때
		//선언부에서 다음과 같이 변경을 해도
	...
	public InternalResourceViewResolver resolver() {
		/*
		 * Controller에서 view를 rendering할 때
		 * 역할을 대신할 클래스
		 */
